<script lang="ts">
  ...
  let pumpControllers = new SvelteMap<string, PumpController>();
  ...
  // Reactive computed values for sizes to ensure reactivity
  const sensorCount = $derived(labState.sensors.size);
  const equipmentCount = $derived(labState.equipment.size);
  const hasData = $derived(sensorCount > 0 || equipmentCount > 0);
  ...
  {#if hasData}
    <div class="dashboard">
      ...
      <section class="equipment">
        <h2>Equipment ({equipmentCount})</h2>
